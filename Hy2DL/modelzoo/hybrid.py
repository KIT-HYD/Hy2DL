...
class Hybrid(nn.Module):
    ...
    def forward(self, x_lstm: torch.Tensor, x_conceptual: torch.Tensor):
        ...
        h0 = torch.zeros(
            self.num_layers, batch_size, self.hidden_size, requires_grad=True, dtype=torch.float32, device=x_lstm.device
        )
        c0 = torch.zeros(
            self.num_layers, batch_size, self.hidden_size, requires_grad=True, dtype=torch.float32, device=x_lstm.device
        )
        ...
        with torch.no_grad():
            pred = self.conceptual_model(
                x_conceptual=x_conceptual[:, : self.warmup_period, :], parameters=parameters_warmup
            )
        # run conceptual model: simulation
        pred = self.conceptual_model(
            x_conceptual=x_conceptual[:, self.warmup_period :, :],
