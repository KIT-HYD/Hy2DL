...
class BaseConceptualModel(nn.Module):
    ...
    def map_parameters(self, lstm_out: torch.Tensor, warmup_period:int) -> Tuple[Dict[str, torch.Tensor], 
        ...
        for index, (parameter_name, parameter_range) in enumerate(self.parameter_ranges.items()):
            ...
            if self.parameter_type[parameter_name]=='static':
                ...
                parameters_warmup[parameter_name] = range_t[:1,:,:] + \
                    torch.sigmoid(lstm_out[:, -1:, index, :].expand(-1, warmup_period, -1)) * \
                        (range_t[1:,:,:] - range_t[:1,:,:])

                parameters_simulation[parameter_name] = range_t[:1,:,:] + \
                    torch.sigmoid(lstm_out[:, -1:, index, :].expand(-1, lstm_out.shape[1]-warmup_period, -1)) * \
                        (range_t[1:,:,:] - range_t[:1,:,:])
 
            elif self.parameter_type[parameter_name]=='dynamic':
                parameters_warmup[parameter_name] = range_t[:1,:,:] + \
                    torch.sigmoid(lstm_out[:, warmup_period-1:warmup_period, index, :].expand(-1, warmup_period, -1)) * \
                          (range_t[1:,:,:] - range_t[:1,:,:])

                parameters_simulation[parameter_name] = range_t[:1,:,:] + \
                    torch.sigmoid(lstm_out[:, warmup_period:, index, :]) * (range_t[1:,:,:] - range_t[:1,:,:])
                
        
        return parameters_warmup, parameters_simulation

    
    def _map_parameter_type(self, parameter_type:Dict[str, str]={}):
        ...
        parameter_type: Dict[str, str]
            Dictionary where the key is the parameter name and the value is the parameter type ('static' or 'dynamic'). 
        ...
        for key, _ in self.parameter_ranges.items():
            if key in parameter_type: # if user specified the type
                map_parameter_type[key] = parameter_type[key]
    ...
    def _get_final_states(self, states: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """Recover final states of the conceptual model
