...
class CAMELS_CH(BaseDataset):
    ...
    def _read_data(self, catch_id: str) -> pd.DataFrame:
        ...
        """
        path_timeseries_obs = (
            Path(self.path_data) / "timeseries" / "observation_based" / f"CAMELS_CH_obs_based_{catch_id}.csv"
        )
        # load time series
        ...
        path_timeseries_sim = (
            Path(self.path_data) / "timeseries" / "simulation_based" / f"CAMELS_CH_sim_based_{catch_id}.csv"
        )
        # load time series
