...
class ModelCalibrationCamelsDE(ModelCalibration):
    def __init__(
        self,
        model,
        path_data: str,
        ...
        super(ModelCalibrationCamelsDE, self).__init__(
            model=model,
            path_data=path_data,
            basin_id=basin_id,
    ...
    def _read_data(self) -> Dict[str, pd.DataFrame]:
        ...
        path_timeseries = self.path_data + "/timeseries/CAMELS_DE_hydromet_timeseries_" + self.basin_id + ".csv"
        # load time series
...
def check_basins_camelsde(
    path_data: str,
    basins_id: List[str],
    target_variables: List[str],
    training_period: Union[List[str], str],
    testing_period: Union[List[str], str],
) -> List[str]:
    ...
    for basin in basins_id:
        # Read information that will be used to optimize the model
        path_timeseries = path_data + "/timeseries/CAMELS_DE_hydromet_timeseries_" + basin + ".csv"
        df = pd.read_csv(path_timeseries)
        df = df.set_index("date")
        df.index = pd.to_datetime(df.index, format="%Y-%m-%d")
