...
os.chdir(sys.path[0])
sys.path.append('..')
from hydrological_models import SHM as hydrological_model # define which model will be calibrated
...
def read_information(path_ts: str, basin_id: str, forcing: List[str], target: List[str], time_period: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Read the information that will be used to run the model.
...
def check_basins(path_ts: str, basin_id: List[str], target: List[str], training_period: List[str], 
                 testing_period: List[str]) -> List[str]:
    ...
    for basin in basins_id:
        # Read information that will be used to optimize the model
        path_timeseries = path_ts + 'CAMELS_GB_hydromet_timeseries_'+ basin+ '_19701001-20150930.csv'
        df = pd.read_csv(path_timeseries)
        df = df.set_index('date')
        df.index = pd.to_datetime(df.index, format="%Y-%m-%d")
        
        df_training = df.loc[training_period[0]:training_period[1], target]
        df_testing = df.loc[testing_period[0]:testing_period[1], target]
...
if __name__ == '__main__': 
    ...
    # Save results
    path_output = '../results/models/conceptual_models_test1/'
    ...
    selected_basins_id = check_basins(path_ts=path_ts, basin_id = basins_id, target=target_variables, 
                                      training_period=training_period, testing_period=testing_period)
