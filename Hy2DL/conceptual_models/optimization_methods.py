...
#
# **References:**
# [1]: "Houska, T., Kraft, P., Chamorro-Chavez, A. and Breuer, L.: SPOTting Model Parameters Using a Ready-Made 
...
class sce:
    ...
    ngs: int
        ...
        analysed parameters
    kstop: int
        the number of past evolution loops and their respective objective value to assess whether the marginal 
        improvement at the current loop (in percentage) is less than pcento
    pcento: float
        the percentage change allowed in the past kstop loops below which convergence is assumed to be achieved.
    peps: float
        Value of the normalized geometric range of the parameters in the population below which convergence is deemed 
        achieved.
    max_loop_inc: int
    ...
    def run_calibration(self, calibration_obj, path_output: str):
        ...
        sampler = spotpy.algorithms.sceua(
            calibration_obj, dbname=file_name, dbformat="csv", save_sim=False, random_state=self.random_state
        )
        sampler.sample(
...
class rope:
    ...
    For a detail explanation of the method see:
    Bárdossy, A. and Singh, S. K.: Robust estimation of hydrological model parameters,  Hydrol. Earth Syst. Sci. 
    Discuss., 5(3), 1641–1675, 2008.
