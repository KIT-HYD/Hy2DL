...
class ModelCalibrationCamelsUS(ModelCalibration):
    def __init__(
        ...
        super(ModelCalibrationCamelsUS, self).__init__(
            model=model,
            path_data=path_data,
            basin_id=basin_id,
    ...
    def _read_data(self) -> Dict[str, pd.DataFrame]:
        ...
        df_target["date"] = pd.to_datetime(
            df_target.Year.map(str) + "/" + df_target.Mnth.map(str) + "/" + df_target.Day.map(str), format="%Y/%m/%d"
        )
...
def check_basins_camelsus(
    path_data: str, basins_id: List[str], training_period: Union[List[str], str], testing_period: Union[List[str], str]
) -> List[str]:
